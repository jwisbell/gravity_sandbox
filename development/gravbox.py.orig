...
p = ArgumentParser()
p.add_argument("-i", "--idle", dest="idle_time", default=60., type=float, help="Set the amount of time (in seconds) until idle mode begins (default 600)")
p.add_argument("-t", "--timing", dest="INT_SECONDS", default=3.5, type=float, help="Set the number of seconds per iteration you would like (default 3.5)")
p.add_argument("-s", "--speed", dest="vel_scaling", type=float, default=1.0, help="Set a scaling factor for the input velocities (default 1.0)")
p.add_argument("-m", "--smoothness", dest="SF", type=int, default=4, help="Smoothness factor for gradient calculation. Lower values make particle more sensitive to noise. Default is 4")
p.add_argument("-d", "--debug", dest="debug", type=int, default=0, help="Use a pre-made density field for testing purposes. Disables tablet I/O. 1 for on, 0 for off.")
p.add_argument("-v", "--verbose", dest="verbose", type=bool, default=False, help="Save a plot displaying the potential field. (default False)")

args = p.parse_args()

waiting = time.time()
ITER = 10000#int(11000 * (args.INT_SECONDS - 2.2))  #number of iteration to orbit over
...
if __name__ == '__main__':
	...
	if args.debug==0:
		...
		call('bash initialize.sh', shell=True)
		previous_pos, previous_vel = io_funcs.read_from_app_2()
	...
	while exit == 0:
		if not idle and args.debug==0:
			...
			dem_file = gdal.Open('/home/gravbox/Desktop/peak.dem')
			# Converts dem_file to numpy array of shape (480, 639)
			scaled_dem_array = np.array(dem_file.GetRasterBand(1).ReadAsArray())/40/SCALE_FACTOR
<<<<<<< HEAD
			display_dem = np.copy(scaled_dem_array)
			np.save('display_dem.npy', display_dem)
...
>>>>>>> 18f8a1198e9930df92457bbb4f3c34105f0c451b
			EDGE = 3
			...
			to_send = gravity_algorithm.run_orbit(particle, 50000, loops=loops,step=0.005,edge_mode='reflect',kind='leapfrog2') #run for 1000 iterations and save the array
			posx = [val[0] for val in to_send]
			posy = [val[1] for val in to_send]	
			active_animating.write_to_file(to_send)
			"""
			Save x,y positions as arrays. 
			"""
			x = np.asarray(posx)
			y = np.asarray(posy)
			np.save('algorithm_output.npy', (x,y))
			#figure.update_fig(display_dem, posy, posx, SCALE_FACTOR)
			#figure.update_fig(scaled_dem_array, SCALE_FACTOR)
		...
		elif not idle and args.debug == 3:
			start = time.time()
			"""
			REFRESH THE DEM FILE SAVED ON DISK
			"""
			...
			dem_file = gdal.Open('/home/gravbox/Desktop/gravboxv2/gravity_sandbox/development/BathymetrySaverTool.dem')
			"""			
			Convert dem_file to np array of shape (480,639).
			"""
			dem_array = np.array(dem_file.GetRasterBand(1).ReadAsArray())
			"""
			Normalize. 
			...
			"""
			Check for new inputs and hand to Particle class.
			"""
			
			if differ(input_pos, previous_pos) or differ(input_vel, previous_vel):
				particle = gravity_algorithm.Particle(input_pos, np.array(input_vel), (gx,gy),g2x,g2y,new=first)
				previous_pos = np.copy(input_pos); previous_vel = np.copy(input_vel)
				waiting = time.time()
			
			first = False
			"""
			Run for 1000 iterations and save the array.
			"""
			to_send = gravity_algorithm.run_orbit(particle, 1000, loops=loops,step=0.005,edge_mode='reflect',kind='leapfrog2') 
			posx = [val[0] for val in to_send]
			posy = [val[1] for val in to_send]	
