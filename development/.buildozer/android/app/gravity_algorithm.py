...
#--------------------------------------------------------------
	...
	try:
       		new_accel = np.nan_to_num(np.array([self.dx[int(new_pos[1]),int(new_pos[0])], self.dy[int(new_pos[1]),int(new_pos[0])]]))
	except:
...
def kepler_check(test_particle,potential, orbits=3,step=0.001,edge_mode='pacman',kind='leapfrog'):
	...
	plt.savefig('./debug/kepler3_v2.png')
