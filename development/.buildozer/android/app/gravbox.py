...
if __name__ == '__main__':
	...
	while exit == 0:
		if not idle and args.debug==0:
			...
			#dem_file = gdal.Open('/home/gravbox/Desktop/smooth.dem')
			# Converts dem_file to numpy array of shape (480, 639)
			#scaled_dem_array = np.array(dem_file.GetRasterBand(1).ReadAsArray())/40/SCALE_FACTOR
			scaled_dem_array,BAD_PIX = topo.update_surface(baseplane, prev_dem)
			prev_dem = scaled_dem_array
			np.save('display_dem.npy',scaled_dem_array)
			...
			gx = np.negative(gx)*.01
			gy = np.negative(gy)*.01
			"""
			gx[BAD_PIX] = np.nan; gy[BAD_PIX] = np.nan

			gx = np.negative(gx)*.100
			gy = np.negative(gy)*.100
			gx[:EDGE,:] = np.nan
			gx[-7:,:] = np.nan
			gx[:,:EDGE] = np.nan
			gx[:,-14:] = np.nan
